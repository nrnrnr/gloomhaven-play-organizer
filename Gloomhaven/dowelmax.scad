...
module stop_pin(small=true) {
  ringdepth = 7; // deeper than original
  translate([0,0,-depth-epsilon])
    cylinder(r = pin_radius(small), h = depth + 2 * epsilon);
...
module allen_key(long, short, outside, inside, thickness) {
  ...
  assert(outside <= inside, "Inside and outside swapped?");
...
module small_allen_key() {
  allen_key(long = allen_key_lengths[0], short=18, thickness=3, inside = 4, outside = 3);
}

module medium_allen_key() {
  allen_key(long = allen_key_lengths[1], short=23, thickness=3.8, inside = 6.0, outside = 6.0);
}
...
module block () {
  ...
  difference() {
    ...
    translate([5+19.6/2,stop_screw_y,0]) // stop pins
      union() {
        stop_pin(small=true);
        translate([small_drill_length-19.6,0,0])
          stop_pin(small=false);
    }
    translate([width-5, 25+right_y_shift, 0]) // shim
      mirror([1,0,0]) shim();
    ...
    if (false) {
      ...
      for (v=ll) {
        translate([width-10,depth-10,0] - 50 * v)
          alignment();
      }
      for (v=ll) {
        translate([width-10,10,0] - 50 * v)
          alignment();
      }
    }
